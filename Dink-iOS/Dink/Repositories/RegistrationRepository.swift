...
struct RegistrationRepository {
    ...
    func fetchSessionRegistrations(sessionId: Int) async throws -> [SessionRegistrationWithUser] {
        let response: [SessionRegistrationWithUser] = try await supabase
            .from("session_registrations")
            .select("*, user_profiles(id, first_name, last_name, email, skill_level, avatar_url)")
            .eq("session_id", value: sessionId)
        ...
        if registration.status == "registered" {
            do {
                ...
                if let first = waitlisted.first {
                    ...
                    try? await NotificationRepository().insertNotification(
                        userId: first.userId,
                        title: "You're In!",
                        body: "A spot opened up — you've been moved off the waitlist.",
                        type: "waitlist_promotion",
                        data: NotificationData(sessionId: sessionId, clubId: nil)
                    )
                }
            } catch {
    ...
    // MARK: - Update Calendar Event ID

    func updateCalendarEventId(registrationId: UUID, eventId: String?) async throws {
