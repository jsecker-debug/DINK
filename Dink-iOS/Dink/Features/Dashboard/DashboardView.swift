    ...
    private var statsGrid: some View {
        LiquidGlassContainer(spacing: 12) {
            LazyVGrid(columns: columns, spacing: 12) {
                StatCardView(title: "Total Members", value: "\(members.count)", icon: "person.2.fill", iconColor: .dinkTeal)
                StatCardView(title: "Active Players", value: "\(activePlayers)", icon: "figure.pickleball", iconColor: .dinkGreen)
                StatCardView(title: "Upcoming", value: "\(upcomingSessions.count)", icon: "calendar", iconColor: .dinkOrange)
                StatCardView(title: "Total Sessions", value: "\(sessions.count)", icon: "list.bullet", iconColor: .dinkNavy)
            }
        }
    }
...
struct SessionRowCard: View {
    ...
    var body: some View {
        HStack(spacing: 12) {
            ...
            if session.parentSessionId != nil || session.isTemplate == true {
