...
struct MemberDetailView: View {
    ...
    var body: some View {
        ...
        .task {
            if let clubId = clubService.selectedClubId {
                attendanceStats = try? await AttendanceRepository().fetchAttendanceStats(userId: member.userId, clubId: clubId)
            }
        }
    }
    ...
    private var statsGrid: some View {
        LiquidGlassContainer(spacing: 12) {
            LazyVGrid(columns: [
                ...
                StatCardView(title: "Games Played", value: "\(member.totalGamesPlayed)", icon: "sportscourt.fill", iconColor: .dinkTeal)
                StatCardView(title: "Wins", value: "\(member.wins)", icon: "arrow.up.right", iconColor: .dinkGreen)
                StatCardView(title: "Losses", value: "\(member.losses)", icon: "arrow.down.right", iconColor: .red)
                StatCardView(title: "Win Rate", value: winRate, icon: "percent", iconColor: .dinkNavy)
