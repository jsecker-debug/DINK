...
final class ClubService {
    ...
    func createClub(
        ...
        let locationArray: [String] = if let location, !location.isEmpty {
            [location]
        } else {
            []
        }
    ...
    /// Fetches all pending join requests for a club (admin view).
    func fetchClubJoinRequests(clubId: UUID) async throws -> [AdminJoinRequest] {
        try await supabase
            .from("club_join_requests")
            .select("id, club_id, user_id, message, status, created_at, user_data:user_profiles(first_name, last_name, email, skill_level)")
    ...
    /// Approves a join request: updates status and creates a club membership.
    func approveJoinRequest(requestId: UUID, clubId: UUID, userId: UUID) async throws {
        try await supabase
            .from("club_join_requests")
